...
for folder in $(ls ${main_folder})
do
  echo $folder
  mkdir -p ${output_folder}/${folder}
  for file in $(ls ${main_folder}/$folder)
  do 
      ...
      echo ${filename_fixed}
      echo ${output_folder}/${folder}/${outfile}

      #check if filename contains 
      # if [[ "$file" == ${prefix}* ]]; then
            ./${build_folder}/main/rawls_h_flip --image ${filename_fixed} --outfile ${output_folder}/${folder}/${outfile}
      # fi
  done 
done